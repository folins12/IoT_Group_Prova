...
| **Previous version (history)** | `<INSERT LINK TO MID-TERM CONCEPT DOCUMENT>` |
...
```
   ...
   └───────────────────────────────────────────────│────────────────┘
                                                    │
                                   ESP-NOW (AES-CCM encrypted,
                                   connectionless, no router, ~1 ms)
                                                    │
                            ┌───────────────────────▼─────────────────────┐
                            │            OBSERVER  (ESP32-S3)              │
                            │  • INA219 #1 → pump current + voltage        │
                            │  • INA219 #2 → auxiliary supply current      │
     ...
     │ live chart +    │                  │  telemetry/state/  │◄────────►│                  │
     │ controls + eval │                  │  alert/cmd topics  │  phone   └──────────────────┘
     └─────────────────┘                  └────────────────────┘  app
```

The **ESP-NOW safety loop is independent** of the WiFi/router/cloud blocks: even if every block to the right of the Observer is unavailable, the pump is still protected.
